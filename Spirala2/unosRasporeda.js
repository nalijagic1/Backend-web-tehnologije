...
function toJSON() {
    var a = [];
    var data = fs.readFileSync('raspored.csv');
...
http.createServer(function (req, res) {
    ...
    if (req.method == 'POST' && req.url == '/raspored') {
        let body = [];
        req.on('data', (chunk) => {
            body.push(chunk);
        }).on('end', () => {
            ...
            if (valid == false) {
                ...
                if (postoji == true) {
                    ...
                    fs.writeFile('raspored.csv', dodati, function (err, data) {
                        if (err) {
    ...
    if (req.method == "GET" && url.parse(req.url,true).pathname==='/raspored') { 
        if (fs.existsSync('raspored.csv') == false) {
